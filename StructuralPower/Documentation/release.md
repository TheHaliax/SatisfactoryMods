...
## Release checklist (3.1.1)

1. **Version fields** in `StructuralPower.uplugin` — `SemVersion` / `VersionName` **3.1.1**, `RemoteVersionRange` **^3.0.0**, `Version` integer **3** (major), `GameVersion` **>=491125**, SML **^3.12.0**.
...
7. **ficsit.app** → New Version → upload `StructuralPower.zip` + changelog **3.1.1**. Refresh mod icon from `Resources/Icon512.png` if asked. Merge `development` → `main` so GitHub README/screenshots match.
8. **Verify** — Mod Manager installs **3.1.1**; client and dedicated share the same mod set.
