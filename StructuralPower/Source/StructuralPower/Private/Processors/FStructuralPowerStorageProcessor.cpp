...
namespace {
void EnsureStorageListener(AStructuralPowerGraphSubsystem& Graph,
...
void FStructuralPowerStorageProcessor::Process(FStructuralPowerContext& Ctx,
                                               AFGBuildablePowerStorage* Storage) {
  if (!IsValid(Storage)) {
    return;
  }
  ...
  if (bBulk) {
    UE_LOG(LogStructuralPower, Verbose,
           ...
           *Storage->GetName(), StructuralEndpointKindToString(EStructuralEndpointKind::Storage),
           StructuralPowerScopeToString(EStructuralPowerScope::Site), Site.SiteRoot,
           StructuralPowerRoleToString(EStructuralPowerRole::Host), Site.SiteRoot,
           Site.bAnchored ? 1 : 0, -1, 0);
  } else {
    UE_LOG(LogStructuralPower, Log,
           ...
           *Storage->GetName(), StructuralEndpointKindToString(EStructuralEndpointKind::Storage),
           StructuralPowerScopeToString(EStructuralPowerScope::Site), Site.SiteRoot,
           StructuralPowerRoleToString(EStructuralPowerRole::Host), Site.SiteRoot,
           Site.bAnchored ? 1 : 0, Outcome.OutletBus ? Outcome.OutletBus->GetCircuitID() : -1,
           Outcome.OutletBus &&
...
void FStructuralPowerStorageProcessor::OnWireDelta(FStructuralPowerContext& Ctx,
                                                   AFGBuildablePowerStorage* Storage) {
  if (!IsValid(Storage)) {
    return;
  }

  FStructuralEndpointAttach::RunStrategy(Ctx, Storage, EStructuralAttachStrategy::Bridge);
}
