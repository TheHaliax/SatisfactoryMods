...
void FStructuralPowerPanelProcessor::TearDown(FStructuralPowerContext& Ctx,
...
void FStructuralPowerPanelProcessor::Process(FStructuralPowerContext& Ctx,
                                             AFGBuildableLightsControlPanel* Panel,
                                             bool bLocalPromoteOnly) {
  HALSP_TRACE_SCOPE_DETAIL(TEXT("mod"), TEXT("panel.Process"),
  ...
  const FStructuralWallAnchor ParentAnchor =
      Ctx.Session().BridgeRootIndex().ResolveOutletAnchor(Panel);
  FStructuralNodeId ParentId;
  const int32 Root =
      Ctx.Session().BridgeRootIndex().ResolveEndpointComponentRoot(Panel, ParentAnchor, ParentId);
  ...
  const bool bRoutingUnchanged = Tracked.bPanelLinksReady && Tracked.CachedPanelRoot == Root &&
  ...
  if (Root != INDEX_NONE) {
    if (IsBulkLoadAttachContext(AttachContext)) {
      Ctx.Session().BridgeRootIndex().AddEndpointToRootIndex(Root, EStructuralEndpointKind::Panel,
                                                             PanelId);
    } else {
  ...
  auto LogPanelState = [&](bool bSupplyReady, const TCHAR* Context) {
    ...
    FStructuralPowerTrace::LogPanelConsumer(Panel, Root, ChannelKey, Ports, bSupplyReady,
                                            Controlled, Context);
  };
  ...
  if (AttachContext == EAttachContext::WireDelta || AttachContext == EAttachContext::Toggle) {
    ...
    if (AttachContext == EAttachContext::Toggle) {
      ...
      if (Ctx.Session().CircuitEcho().ShouldSkipPanelCircuitEcho(Panel, &SkipReason) &&
          SkipReason && FCString::Strcmp(SkipReason, TEXT("skip_feed_open")) == 0) {
    ...
    if (AttachContext == EAttachContext::WireDelta && Tracked.bPanelLinksReady &&
      ...
      if (FStructuralPanelAttach::SupplyAlreadyLinked(Ctx.Session(), Panel, Ports, Root,
                                                      ChannelKey)) {
        ...
        UE_LOG(LogStructuralPower, Log,
               ...
               *Panel->GetName(), StructuralEndpointKindToString(EStructuralEndpointKind::Panel),
               ...
               AttachContextToString(AttachContext), Root,
  ...
  if (Root != INDEX_NONE && !FeedSwitchId.IsNone() &&
  ...
  if (bRoutingUnchanged) {
    ...
    const bool bSupplyLinked =
        FStructuralPanelAttach::SupplyAlreadyLinked(Ctx.Session(), Panel, Ports, Root, ChannelKey);
    const UFGPowerConnectionComponent* InputPower =
        FStructuralPanelPortResolver::AsPowerConnection(Ports.Input);
    const bool bInputLive = bSupplyLinked && IsValid(InputPower) &&
  ...
  if (Root != INDEX_NONE) {
    ...
    if (bFeedReady) {
      if (FStructuralPanelAttach::SupplyAlreadyLinked(Ctx.Session(), Panel, Ports, Root,
        ...
        bSupplyReady = FStructuralPanelAttach::TryLinkSupply(Ctx.Session(), Panel, Ports, Root,
                                                             ChannelKey, bMeshOnlySupply);
      }
    }
  }
  ...
  Tracked.bPanelLinksReady =
      IsValid(InputPower) && FStructuralCircuitPromotionUtil::ConnectorSuppliesPower(InputPower);
...
void FStructuralPowerPanelProcessor::OnWireDelta(FStructuralPowerContext& Ctx,
                                                 AFGBuildableLightsControlPanel* Panel) {
