	...
	FTrackedEndpoint& Tracked)
{
	...
	if (FStructuralPowerTrace::IsEnabled())
	{
		UE_LOG(LogStructuralPower, Log,
			TEXT("[HALSP] panel transfer_off %s attach=%s"),
			*Panel->GetName(),
			AttachContextToString(Ctx.GetAttachContext()));
	}
}

void SuspendLightTransfer(
	FStructuralPowerContext& Ctx,
	...
	FTrackedEndpoint& Tracked)
{
	...
	{
		UE_LOG(LogStructuralPower, Log,
			TEXT("[HALSP] light transfer_off %s attach=%s"),
			*Light->GetName(),
			AttachContextToString(Ctx.GetAttachContext()));
	}
}
...
{
	return AttachContext == EAttachContext::WireDelta
...
void FStructuralPowerTransferGate::FlipBridgeGate(
	FStructuralPowerContext& Ctx,
...
void FStructuralPowerTransferGate::ApplyKeyedTransferOnRoot(
	FStructuralPowerContext& Ctx,
	int32 Root,
	...
	bool bLocalPromoteOnly)
{
	...
	{
		UE_LOG(LogStructuralPower, Log,
			TEXT("[HALSP] keyed transfer refresh root=%d attach=%s"),
			Root,
			AttachContextToString(Ctx.GetAttachContext()));
	}
