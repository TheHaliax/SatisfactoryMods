...
void FStructuralPowerPoleProcessor::ResolvePoleStructuralSite(
  ...
  if (FStructuralSiteMembership::ResolveSiteContext(Session, Pole, Site,
...
void FStructuralPowerPoleProcessor::Process(FStructuralPowerContext& Ctx,
                                            AFGBuildablePowerPole* Pole) {
  HALSP_TRACE_SCOPE_DETAIL(TEXT("mod"), TEXT("pole.Process"),
  ...
  const int32 Powered =
      Outcome.OutletBus && FStructuralCircuitPromotionUtil::ComponentCarriesPower(Outcome.OutletBus)
          ? 1
          : 0;
  ...
  if (bBulk) {
    UE_LOG(LogStructuralPower, Verbose,
           ...
           *Pole->GetName(), StructuralEndpointKindToString(EStructuralEndpointKind::Pole),
           StructuralPowerScopeToString(EStructuralPowerScope::Site), Site.SiteRoot,
           StructuralPowerRoleToString(EStructuralPowerRole::Router), Site.SiteRoot,
           bStructurallyAnchored ? 1 : 0,
           FStructuralOutletParentResolver::FormatParentMethod(Site.ParentMethod), ParentDistCm,
           PowerPath, BusCircuit, Powered, StructuralChannelToString(EStructuralChannel::Structure),
           TEXT("-"));
  } else {
...
void FStructuralPowerPoleProcessor::OnWireDelta(FStructuralPowerContext& Ctx,
                                                AFGBuildablePowerPole* Pole) {
  if (!IsValid(Pole)) {
    return;
  }

  FStructuralEndpointAttach::RunStrategy(Ctx, Pole, EStructuralAttachStrategy::Bridge);
}
