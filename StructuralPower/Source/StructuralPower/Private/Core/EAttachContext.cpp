...
const TCHAR* AttachContextToString(const EAttachContext Context)
{
	switch (Context)
	{
	case EAttachContext::BulkLoad: return TEXT("bulk");
	case EAttachContext::RuntimePlace: return TEXT("runtime");
	case EAttachContext::WireDelta: return TEXT("wire_delta");
	case EAttachContext::Toggle: return TEXT("toggle");
	default: return TEXT("?");
	}
}
