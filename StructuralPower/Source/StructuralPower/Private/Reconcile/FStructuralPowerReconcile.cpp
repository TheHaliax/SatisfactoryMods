...
void FStructuralPowerReconcile::RunPostLoadMachineWorkers() {
  if (!Session) {
    return;
  }
  ...
  if (FStructuralPowerModConfig::IsGroupResourcesEnabled()) {
    ReconcileGroupResourcesState();
  }
  if (FStructuralPowerModConfig::IsGroupProductionEnabled()) {
    ReconcileGroupProductionState();
  }
  if (FStructuralPowerModConfig::IsGroupTransportEnabled()) {
    ReconcileGroupTransportState();
  }
  if (FStructuralPowerModConfig::IsGroupPipesEnabled()) {
    ReconcileGroupPipesState();
  }
}
...
bool FStructuralPowerReconcile::PanelNeedsLightingReconcileRetry(
  ...
  if (!FStructuralPanelAttach::SupplyAlreadyLinked(*Session, Panel, Ports, Root, ChannelKey)) {
    return true;
  }
...
bool FStructuralPowerReconcile::IsDirectSwitchFedLight(int32 Root,
    ...
    for (const FStructuralNodeId& NodeId : SwitchIdsSnapshot) {
      if (const FTrackedEndpoint* Tracked = Session->TrackedEndpoints().Find(NodeId)) {
        if (AFGBuildableCircuitSwitch* Switch = Tracked->GetSwitch()) {
          if (Session->Ids().ResolveControl(Switch, EStructuralChannel::Switch) ==
              LightKey.Source) {
            return true;
          }
        }
...
bool FStructuralPowerReconcile::IsPanelDownstreamLight(int32 Root,
                                                       const FStructuralChannelKey& LightKey) {
...
bool FStructuralPowerReconcile::IsSwitchFeedOpen(int32 Root, FName SwitchControlId) {
...
void FStructuralPowerReconcile::LogPanelReconcileSummary(AFGBuildableLightsControlPanel* Panel) {
  if (!IsValid(Panel)) {
    return;
  }
  ...
  const UFGPowerConnectionComponent* InputPower =
      FStructuralPanelPortResolver::AsPowerConnection(Ports.Input);
  const bool bSupplyReady = FStructuralCircuitPromotionUtil::ConnectorSuppliesPower(InputPower);
  const int32 Controlled =
      Panel->GetControlledBuildables(AFGBuildableLightSource::StaticClass()).Num();

  FStructuralPowerTrace::LogPanelConsumer(Panel, Root, ChannelKey, Ports, bSupplyReady, Controlled,
                                          TEXT("reconcile"));
}
...
void FStructuralPowerReconcile::CollectKnownPanelEndpoints(
  ...
  Session->EndpointIndex().ForEachEndpoint(
      EStructuralEndpointKind::Panel, [&](const FStructuralNodeId& PanelId) {
        if (const FTrackedEndpoint* Tracked = Session->TrackedEndpoints().Find(PanelId)) {
...
void FStructuralPowerReconcile::ReconcileAllPanelEndpoints() {
  ...
  if (FStructuralPowerModConfig::IsGroupLightingEnabled()) {
    UE_LOG(LogStructuralPower, Log,
           TEXT("[HALSP] Post-load panel reconcile complete — %d panel(s)"), Panels.Num());
...
void FStructuralPowerReconcile::RefreshKeyedTransferAfterLoad() {
    ...
    if (const TArray<FStructuralNodeId>* PanelIds =
      ...
      for (const FStructuralNodeId& PanelId : *PanelIds) {
        if (FTrackedEndpoint* PanelTracked = Session->TrackedEndpoints().Find(PanelId)) {
          if (Session->Ids().ResolveSource(PanelTracked->GetPanel(), EStructuralChannel::Light) !=
              Control) {
            continue;
          }
  ...
  for (int32 Root : Roots) {
    FStructuralPowerContext Ctx = Session->MakeProcessorContext(EAttachContext::RuntimePlace, Root);
    ...
    if (Root != INDEX_NONE && !Source.IsNone()) {
      if (const TArray<FStructuralNodeId>* SwitchIds =
        ...
        for (const FStructuralNodeId& SwitchId : *SwitchIds) {
          const FTrackedEndpoint* SwitchTracked = Session->TrackedEndpoints().Find(SwitchId);
          if (!SwitchTracked) {
            continue;
          }
...
void FStructuralPowerReconcile::SuspendAllIntegratedLighting() {
  ...
  UE_LOG(LogStructuralPower, Log,
         TEXT("[HALSP] suspended integrated lighting — %d panel(s) %d light(s)"), Panels.Num(),
         Lights.Num());
}
...
void FStructuralPowerReconcile::ReconcileGroupLightingState() {
  ...
  UE_LOG(LogStructuralPower, Log, TEXT("[HALSP] GroupLighting reconcile — enabled=%d"),
         bOn ? 1 : 0);
...
void FStructuralPowerReconcile::ReconcileGroupGenerationState() {
  const bool bOn = FStructuralPowerModConfig::IsGroupGenerationEnabled();
  UE_LOG(LogStructuralPower, Log, TEXT("[HALSP] GroupGeneration reconcile — enabled=%d"),
         bOn ? 1 : 0);
  ...
  for (const TPair<FStructuralNodeId, FTrackedEndpoint>& Pair : Session->TrackedEndpoints()) {
    ...
    if (Pair.Value.Kind != EStructuralEndpointKind::Generator &&
    ...
    ++Touched;
    if (bOn) {
      FStructuralEndpointDispatcher::DispatchPlacement(*Session, Host);
    } else {
  ...
  if (AFGBuildableSubsystem* BuildableSubsystem = AFGBuildableSubsystem::Get(World)) {
    for (AFGBuildable* Buildable : BuildableSubsystem->GetAllBuildablesRef()) {
      if (FStructuralEligibilityRules::IsStructuralGenerator(Buildable) ||
          FStructuralEligibilityRules::IsPowerStorage(Buildable)) {
        FStructuralEndpointDispatcher::DispatchPlacement(*Session, Buildable);
      }
    }
  }
...
void FStructuralPowerReconcile::ReconcileGroupKindState(const EStructuralEndpointKind Kind,
                                                        const bool bEnabled, const TCHAR* Label) {
  UE_LOG(LogStructuralPower, Log, TEXT("[HALSP] %s reconcile — enabled=%d"), Label,
         bEnabled ? 1 : 0);
  ...
  for (const TPair<FStructuralNodeId, FTrackedEndpoint>& Pair : Session->TrackedEndpoints()) {
    AFGBuildable* Host = Pair.Value.Actor.Get();
    if (!IsValid(Host) || Pair.Value.Kind != Kind) {
      continue;
    }
...
void FStructuralPowerReconcile::ReconcileGroupResourcesState() {
  ReconcileGroupKindState(EStructuralEndpointKind::Extractor,
                          FStructuralPowerModConfig::IsGroupResourcesEnabled(),
                          TEXT("GroupResources"));
}

void FStructuralPowerReconcile::ReconcileGroupProductionState() {
  ReconcileGroupKindState(EStructuralEndpointKind::Manufacturer,
                          FStructuralPowerModConfig::IsGroupProductionEnabled(),
                          TEXT("GroupProduction"));
}

void FStructuralPowerReconcile::ReconcileGroupTransportState() {
  ReconcileGroupKindState(EStructuralEndpointKind::Transport,
...
void FStructuralPowerReconcile::ReconcileAllLightConsumers() {
  ...
  UE_LOG(LogStructuralPower, Log, TEXT("Reconcile lights — GroupLighting=%d candidate(s)=%d"),
         FStructuralPowerModConfig::IsGroupLightingEnabled() ? 1 : 0, Lights.Num());
...
void FStructuralPowerReconcile::RefreshPanelsForLightSourceOnRoot(int32 Root, FName LightSource) {
  ...
  for (const TPair<FStructuralNodeId, FTrackedEndpoint>& Pair : Session->TrackedEndpoints()) {
    ...
    const FStructuralWallAnchor Anchor =
        Session->BridgeRootIndex().ResolveOutletAnchor(Pair.Value.GetPanel());
    FStructuralNodeId ParentId;
    if (Session->BridgeRootIndex().ResolveEndpointComponentRoot(Pair.Value.GetPanel(), Anchor,
                                                                ParentId) != Root) {
      continue;
    }
