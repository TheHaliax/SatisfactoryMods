...
static void FillIntSelector(UFGUserSetting_IntSelector* Selector, const TArray<FName>& Options, int32 SelectedIndex)
{
	if (!IsValid(Selector))
	{
		return;
...
void UStructuralPowerIdOptionManager::SyncFromComponentList(const FStructuralComponentIdList& List)
{
		...
		|| TargetBuildable.IsA<AFGBuildableLightsControlPanel>())
	{
		...
		const FName Active = List.ControlOverride.IsNone() ? List.ResolvedControl : List.ControlOverride;
...
UFGUserSetting* UStructuralPowerIdOptionManager::CreateDropdownSetting(
...
void UStructuralPowerIdOptionManager::RegisterSetting(UFGUserSetting* Setting)
{
	...
	if (UFGUserSettingApplyType* ApplyType = UFGUserSettingApplyType::GetUserSettingApplyType(this, Setting))
	{
...
void UStructuralPowerIdOptionManager::PopulateSelector(
	UFGUserSetting* Setting,
	const TArray<FName>& Options,
	int32 SelectedIndex)
{
	if (!IsValid(Setting))
	...
	if (UFGUserSetting_IntSelector* Selector = Cast<UFGUserSetting_IntSelector>(Setting->ValueSelector))
	{
...
UFGUserSettingApplyType* UStructuralPowerIdOptionManager::FindUserSetting(const FString& SettingId) const
{
...
IFGOptionInterface* UStructuralPowerIdOptionManager::GetPrimaryOptionInterface(UWorld* World) const
{
	...
	UUserWidget* OwningWidget)
{
	...
	TScriptInterface<IFGOptionInterface> OptionInterface(const_cast<UStructuralPowerIdOptionManager*>(this));
	return UFGOptionsLibrary::GetCategorizedUserSettingsWidgets(
		WorldContext,
		OwningWidget,
		OptionInterface,
		SettingsMap);
