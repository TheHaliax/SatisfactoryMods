...
void FStructuralPowerIdShellBuilder::ResetShellState(TObjectPtr<UWidgetTree>& InWidgetTree,
...
void FStructuralPowerIdShellBuilder::EnsureShellBuilt(UUserWidget* Widget,
  ...
  UOverlay* RootOverlay =
      InWidgetTree->ConstructWidget<UOverlay>(UOverlay::StaticClass(), TEXT("RootOverlay"));
  RootOverlay->SetVisibility(ESlateVisibility::Visible);
  RootOverlay->SetClipping(EWidgetClipping::ClipToBoundsAlways);

  UBorder* Backdrop =
      InWidgetTree->ConstructWidget<UBorder>(UBorder::StaticClass(), TEXT("ModalBackdrop"));
  ...
  USizeBox* Frame =
      InWidgetTree->ConstructWidget<USizeBox>(USizeBox::StaticClass(), TEXT("RootFrame"));
  ...
  Shell.ShellBorder =
      InWidgetTree->ConstructWidget<UBorder>(UBorder::StaticClass(), TEXT("ShellBorder"));
  ...
  UVerticalBox* ShellVBox =
      InWidgetTree->ConstructWidget<UVerticalBox>(UVerticalBox::StaticClass(), TEXT("ShellVBox"));
  Shell.ShellBorder->AddChild(ShellVBox);

  UBorder* TitleBar =
      InWidgetTree->ConstructWidget<UBorder>(UBorder::StaticClass(), TEXT("TitleBar"));
  ...
  UHorizontalBox* TitleRow = InWidgetTree->ConstructWidget<UHorizontalBox>(
      UHorizontalBox::StaticClass(), TEXT("TitleRow"));
  TitleBar->AddChild(TitleRow);

  Shell.TitleText =
      InWidgetTree->ConstructWidget<UTextBlock>(UTextBlock::StaticClass(), TEXT("TitleText"));
  ...
  Shell.ContentBorder =
      InWidgetTree->ConstructWidget<UBorder>(UBorder::StaticClass(), TEXT("ContentBorder"));
  Shell.ContentBorder->SetBrushColor(FLinearColor(0.05f, 0.07f, 0.09f, 0.98f));
  Shell.ContentBorder->SetPadding(FMargin(18.0f));

  Shell.ContentVBox =
      InWidgetTree->ConstructWidget<UVerticalBox>(UVerticalBox::StaticClass(), TEXT("ContentVBox"));
  ...
  UE_LOG(LogStructuralPower, Log, TEXT("[HALSP] Id panel shell built root=%s chrome=cpp"),
         *RootOverlay->GetName());
}

void FStructuralPowerIdShellBuilder::UpdateWindowTitle(const FStructuralPowerIdShellWidgets& Shell,
  ...
  SetTextStyle(Shell.TitleText, FString::Printf(TEXT("Structural Power — %s"), Kind), 15, true);
}

void FStructuralPowerIdShellBuilder::EnsureViewportPresentation(
