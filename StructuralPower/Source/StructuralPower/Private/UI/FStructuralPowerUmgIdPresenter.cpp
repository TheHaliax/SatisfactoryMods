...
void FStructuralPowerUmgIdPresenter::ApplyEndpointIds(AFGBuildable* Target, const FName Source,
                                                      const FName Control, const bool bClearSource,
                                                      const bool bClearControl) {
  if (AFGPlayerController* PC = PreparedController.Get()) {
    FStructuralPowerIdApplyBridge::ApplyEndpointIds(PC, Target, Source, Control, bClearSource,
                                                    bClearControl);
    return;
  }
...
void FStructuralPowerUmgIdPresenter::ForceReleaseModalState(AFGPlayerController* PlayerController,
                                                            const bool bRestoreGameInputMode) {
  UStructuralPowerIdConfigWidget::ForceReleaseAllModalState(PlayerController,
                                                            bRestoreGameInputMode);
}

void FStructuralPowerUmgIdPresenter::ReleaseForVanillaInteract(
