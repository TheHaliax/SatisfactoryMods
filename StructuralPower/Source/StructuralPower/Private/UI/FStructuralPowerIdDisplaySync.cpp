...
void FStructuralPowerIdDisplaySync::ApplyComponentIdList(UStructuralPowerIdConfigWidget* Widget,
...
void FStructuralPowerIdDisplaySync::RepopulateComboFromManager(
    UComboBoxString* Combo, UStructuralPowerIdOptionManager* OptionManager, bool bSourceChannel) {
  if (!IsValid(Combo) || !IsValid(OptionManager)) {
    return;
  }

  Combo->ClearOptions();
  const int32 Count = bSourceChannel ? OptionManager->GetSourceOptionCount()
                                     : OptionManager->GetControlOptionCount();
  for (int32 Index = 0; Index < Count; ++Index) {
    const FName Id = bSourceChannel ? OptionManager->GetSourceOptionAt(Index)
...
void FStructuralPowerIdDisplaySync::RefreshIdDisplayFromList(
    UStructuralPowerIdConfigWidget* Widget, const FStructuralComponentIdList& List) {
  ...
  UE_LOG(LogStructuralPower, Log,
         TEXT("[HALSP] Id panel synced ids resolvedSrc=%s resolvedCtl=%s overrideSrc=%s "
              "overrideCtl=%s"),
         List.ResolvedSource.IsNone() ? TEXT("-") : *List.ResolvedSource.ToString(),
         List.ResolvedControl.IsNone() ? TEXT("-") : *List.ResolvedControl.ToString(),
