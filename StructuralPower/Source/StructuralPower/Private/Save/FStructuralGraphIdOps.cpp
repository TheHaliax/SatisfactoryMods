...
FStructuralComponentKey FStructuralGraphIdOps::MakeComponentKeyForBuildable(
  ...
  const FStructuralWallAnchor Anchor = FStructuralAttachmentResolver::ResolveStructuralParent(
      const_cast<AFGBuildable*>(Buildable), Session->GetWorld(), Session->LightweightIndex());
  if (Anchor.IsValid()) {
...
bool FStructuralGraphIdOps::GetEndpointOverrides(const AFGBuildable* Buildable,
  ...
  const FStructuralNodeId BuildableId = Session->MakeNodeId(Buildable);
  if (const FStructuralEndpointOverrides* Found =
...
FName FStructuralGraphIdOps::ResolveSource(AFGBuildable* Buildable, EStructuralChannel Tag) {
...
FName FStructuralGraphIdOps::ResolveControl(AFGBuildable* Buildable, EStructuralChannel Tag) {
  if (!IsValid(Buildable)) {
    return NAME_None;
  }
...
void FStructuralGraphIdOps::SetEndpointIds(AFGBuildable* Buildable, FName Source, FName Control,
  ...
  Session->IdRegistry().ApplyPlayerOverrideEdits(
      NodeId, Source, Control, bClearSource, bClearControl, bGlobalControl, bTouchGlobalControl);
  ...
  const int32 Root =
      Session->BridgeRootIndex().ResolveEndpointComponentRoot(Buildable, Anchor, ParentId);
  if (Root == INDEX_NONE) {
    return;
  }
  ...
  if (bIsLight && bGroupLighting) {
    ...
    if (AFGBuildableCircuitSwitch* Switch = FStructuralPowerBuildableCasts::AsSwitch(Buildable)) {
      FStructuralEndpointDispatcher::DispatchPlacement(*Session, Switch,
                                                       /*bLocalPromoteOnly=*/false,
                                                       /*bOverrideAttachContext=*/true,
                                                       EAttachContext::RuntimePlace);
    }
...
bool FStructuralGraphIdOps::CollectIdsOnComponent(const FStructuralComponentKey& Key,
  ...
  Out = {};
  Out.DefaultSourceId =
      const_cast<FStructuralGraphIdOps*>(this)->GetOrCreateComponentDefaultId(Key);
  ...
  auto ConsiderBuildable = [&](const FStructuralNodeId& NodeId, const AFGBuildable* Buildable) {
    ...
    if (const FStructuralEndpointOverrides* Overrides =
            Session->IdRegistry().FindPlayerOverride(NodeId)) {
      if (!Overrides->SourceOverride.IsNone() && Overrides->SourceOverride != Out.DefaultSourceId) {
    ...
    if (bIsSwitch) {
      const FName TagControl = FStructuralPowerRouter::ResolveSwitchControlFromTag(
          Cast<AFGBuildableCircuitSwitch>(Buildable));
      if (!TagControl.IsNone() && !FStructuralPowerRouter::IsReservedSentinel(TagControl)) {
    ...
    FStructuralGraphIdOps* MutableThis = const_cast<FStructuralGraphIdOps*>(this);
    ...
    if (FStructuralPowerRouter::IsPlayerChosenIdValid(ResolvedSource) &&
        ResolvedSource != Out.DefaultSourceId && !bIsLight) {
  ...
  for (const TPair<FStructuralNodeId, FTrackedEndpoint>& Pair : Session->TrackedEndpoints()) {
    ...
    const FName LightSource =
        const_cast<FStructuralGraphIdOps*>(this)->ResolveSource(Light, EStructuralChannel::Light);
