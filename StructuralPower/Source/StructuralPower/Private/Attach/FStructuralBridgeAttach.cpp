...
bool FStructuralBridgeAttach::HasPlacementMembership(FStructuralGraphSession& Session,
                                                     AFGBuildable* Host,
  ...
  return Tracked && Tracked->Kind == ExpectedKind && Tracked->MountParentId.IsValid();
}

namespace {
bool TryIdleSkip(FStructuralGraphSession& Session, AFGBuildable* Host, EStructuralEndpointKind Kind,
                 const EAttachContext AttachContext) {
  if (Kind != EStructuralEndpointKind::Pole || Session.IsBulkLoadDrainActive() ||
  ...
  if (!Existing || Existing->Kind != EStructuralEndpointKind::Pole ||
      !IsValid(Session.FindBusConnector(Host)) || Existing->MountParentId.IsValid() ||
      Existing->bAwaitingStructuralSite || FStructuralPoleWireUtil::HasVanillaWire(Pole)) {
    return false;
  }
...
FStructuralBridgeAttachOutcome RefreshWireDeltaMembership(
  ...
  if (!FStructuralSiteMembership::ResolveSiteContext(Session, Host, Site,
  ...
  if (Site.bAnchored && Site.SiteRoot != INDEX_NONE) {
    FStructuralSiteMembership::IntegrateOnPlace(Session, Host, OutletBus, EndpointId, Request.Kind,
                                                Tracked, Site, Params);
...
FStructuralBridgeAttachOutcome FStructuralBridgeAttach::AttachOnPlace(
  ...
  if (!FStructuralSiteMembership::ResolveSiteContext(Session, Host, Site,
    ...
    if (Request.bUsePoleRootResolver) {
      ...
      const FStructuralWallAnchor ParentAnchor =
          Session.BridgeRootIndex().ResolveOutletAnchor(Host);
      Session.BridgeRootIndex().ResolveEndpointComponentRoot(Host, ParentAnchor,
                                                             Site.MountParentId);
  ...
  if (bBulk) {
    FStructuralSiteMembership::RegisterOnBulkLoad(Session, Host, Request.Kind, Tracked, Site);
  ...
  Outcome.OutletBus = OutletBus;
  if (!OutletBus) {
    FStructuralPowerTrace::LogPlacementSkip(Host, TEXT("bridge_bus_create_failed"),
