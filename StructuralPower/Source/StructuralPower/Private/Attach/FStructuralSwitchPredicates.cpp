...
bool FStructuralSwitchPredicates::HasAssignedControl(const FStructuralPowerContext& Ctx,
  ...
  const FName Control = const_cast<FStructuralPowerContext&>(Ctx).Session().Ids().ResolveControl(
      const_cast<AFGBuildableCircuitSwitch*>(Switch), EStructuralChannel::Switch);
  return FStructuralPowerRouter::IsAssignedControl(Control);
}

bool FStructuralSwitchPredicates::NeedsAdvancedWork(const FStructuralPowerContext& Ctx,
                                                    const AFGBuildableCircuitSwitch* Switch) {
  if (!IsValid(Switch)) {
    return false;
  }

  return HasAssignedControl(Ctx, Switch) || FStructuralSwitchParentResolver::HasAnyVanillaWire(
                                                const_cast<AFGBuildableCircuitSwitch*>(Switch));
}

bool FStructuralSwitchPredicates::ShouldInjectStructuralPath(
    const AFGBuildableCircuitSwitch* Switch) {
