...
void FStructuralPowerDiagnostics::AuditWorld(UWorld* World, bool bAllowMenuWorld) {
  ...
  if (Graph) {
    ...
    UE_LOG(LogStructuralPower, Log,
           TEXT("Graph: %d structure node(s), %d component(s) (largest %d), %d tracked pole(s), %d "
                "LW tracked, %d pending, propagation=%d trace=%d"),
           Graph->GetStructureNodeCount(), Components, Largest, Graph->GetTrackedPoleCount(),
           Graph->GetTrackedLightweightCount(), Graph->GetPendingJobCount(),
  ...
  UE_LOG(LogStructuralPower, Log,
         TEXT("Bridge poles: %d total | WITH_BUS: %d | IN_CIRCUIT: %d | NO_BUS: %d"), PolesTotal,
         PolesWithBus, PolesInCircuit, PolesNoBus);
  UE_LOG(LogStructuralPower, Log,
         TEXT("Pole-to-pole: WITH_BUS = tracked pole (bus created). IN_CIRCUIT = bus promoted to a "
              "power circuit (component powered)."));
}
