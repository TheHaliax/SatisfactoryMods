...
void PushHook(const TCHAR* Op, const FString& Detail)
{
	...
	UE_LOG(LogStructuralPower, Log,
		TEXT("[HALSP] vanilla> %s frame=%d depth=%d %s"),
		Op,
		FStructuralPowerTraceUtil::GetFrameNumber(),
		GVanillaHookStack.Num() - 1,
		*Detail);
}
...
void PopHook()
{
	...
	const double ElapsedMs = FPlatformTime::ToMilliseconds64(
		FPlatformTime::Cycles64() - Frame.StartCycles);

	UE_LOG(LogStructuralPower, Log,
		TEXT("[HALSP] vanilla< %s ms=%.3f frame=%d depth=%d %s"),
		Frame.Op ? Frame.Op : TEXT("?"),
		ElapsedMs,
		FStructuralPowerTraceUtil::GetFrameNumber(),
		GVanillaHookStack.Num(),
		*Frame.Detail);
}

FString PairDetail(
	const TCHAR* LabelA,
	const UFGCircuitConnectionComponent* A,
	const TCHAR* LabelB,
	const UFGCircuitConnectionComponent* B)
{
	return FString::Printf(
		TEXT("%s={%s} %s={%s}"),
		LabelA,
		*FStructuralVanillaPowerTrace::FormatConnector(A),
		LabelB,
		*FStructuralVanillaPowerTrace::FormatConnector(B));
}
} // namespace

FString FStructuralVanillaPowerTrace::FormatConnector(const UFGCircuitConnectionComponent* Connector)
{
	...
	return FString::Printf(
		TEXT("%s.%s hidden=%d circuit=%d wires=%d hiddenN=%d"),
		IsValid(Owner) ? *Owner->GetName() : TEXT("?"),
		*Connector->GetName(),
		Connector->IsHidden() ? 1 : 0,
		Connector->GetCircuitID(),
		Connector->GetNumConnections(),
		Connector->GetNumHiddenConnections());
}
...
void FStructuralVanillaPowerTrace::OnConnectComponentsEnter(
	AFGCircuitSubsystem* Subsystem,
...
void FStructuralVanillaPowerTrace::OnConnectComponentsExit(
	AFGCircuitSubsystem* /*Subsystem*/,
	UFGCircuitConnectionComponent* First,
	UFGCircuitConnectionComponent* Second)
{
...
void FStructuralVanillaPowerTrace::OnAddHiddenConnectionEnter(
	UFGCircuitConnectionComponent* Self,
	UFGCircuitConnectionComponent* Other)
{
	...
	PushHook(
		TEXT("AddHiddenConnection"),
		PairDetail(TEXT("self"), Self, TEXT("other"), Other));
}

void FStructuralVanillaPowerTrace::OnAddHiddenConnectionExit(
	UFGCircuitConnectionComponent* Self,
	UFGCircuitConnectionComponent* Other)
{
	if (!ShouldLog())
...
void FStructuralVanillaPowerTrace::OnAddWireConnectionEnter(
	UFGCircuitConnectionComponent* Self,
	AFGBuildableWire* Wire)
{
	...
	PushHook(
		...
		FString::Printf(
			TEXT("conn={%s} wire=%s"),
			*FormatConnector(Self),
			IsValid(Wire) ? *Wire->GetName() : TEXT("null")));
}

void FStructuralVanillaPowerTrace::OnAddWireConnectionExit(
	UFGCircuitConnectionComponent* Self,
	AFGBuildableWire* Wire)
{
	if (!ShouldLog())
	...
	if (GVanillaHookStack.Num() > 0)
	{
		GVanillaHookStack.Last().Detail = FString::Printf(
			TEXT("conn={%s} wire=%s circuit=%d"),
			*FormatConnector(Self),
			IsValid(Wire) ? *Wire->GetName() : TEXT("null"),
...
void FStructuralVanillaPowerTrace::OnRemoveWireConnectionEnter(
	UFGCircuitConnectionComponent* Self,
	AFGBuildableWire* Wire)
{
	if (!ShouldLog())
...
void FStructuralVanillaPowerTrace::OnRemoveWireConnectionExit(
	UFGCircuitConnectionComponent* Self,
	AFGBuildableWire* Wire)
{
	if (!ShouldLog())
...
void FStructuralVanillaPowerTrace::OnSetCircuitBridgesModified(AFGCircuitSubsystem* Subsystem)
{
	...
	UE_LOG(LogStructuralPower, Log,
		TEXT("[HALSP] vanilla evt SetCircuitBridgesModified subsystem=%p frame=%d"),
		Subsystem,
		FStructuralPowerTraceUtil::GetFrameNumber());
}
...
void FStructuralVanillaPowerTrace::OnSetSwitchOn(AFGBuildableCircuitSwitch* Switch, bool bOn)
{
	...
	UE_LOG(LogStructuralPower, Log,
		...
		*FormatBuildable(Switch),
		bOn ? 1 : 0,
		IsValid(Switch) && Switch->IsBridgeActive() ? 1 : 0,
		FStructuralPowerTraceUtil::GetFrameNumber());
}
...
void FStructuralVanillaPowerTrace::OnBridgeCircuitsRebuilt(AFGBuildableCircuitBridge* Bridge)
{
	...
	UE_LOG(LogStructuralPower, Log,
		TEXT("[HALSP] vanilla evt OnCircuitsRebuilt %s frame=%d"),
		*FormatBuildable(Bridge),
		FStructuralPowerTraceUtil::GetFrameNumber());
}
...
{
	SUBSCRIBE_METHOD(
		AFGCircuitSubsystem::ConnectComponents,
		[](auto& /*Scope*/,
			AFGCircuitSubsystem* Subsystem,
			UFGCircuitConnectionComponent* First,
			UFGCircuitConnectionComponent* Second)
	...
	SUBSCRIBE_METHOD(
		AFGCircuitSubsystem::DisconnectComponents,
		[](auto& /*Scope*/,
			AFGCircuitSubsystem* Subsystem,
			UFGCircuitConnectionComponent* First,
			UFGCircuitConnectionComponent* Second)
	...
	SUBSCRIBE_METHOD_AFTER(
		...
		[](AFGCircuitSubsystem* Subsystem)
		{
			OnSetCircuitBridgesModified(Subsystem);
		});
	...
	SUBSCRIBE_METHOD(
		...
		[](auto& /*Scope*/,
			...
			UFGCircuitConnectionComponent* Other)
		{
			OnAddHiddenConnectionEnter(Self, Other);
		});
	...
	SUBSCRIBE_METHOD(
		...
		[](auto& /*Scope*/,
			...
			UFGCircuitConnectionComponent* Other)
		{
			OnRemoveHiddenConnectionEnter(Self, Other);
		});
	...
	SUBSCRIBE_METHOD(
		UFGCircuitConnectionComponent::AddConnection,
		[](auto& /*Scope*/, UFGCircuitConnectionComponent* Self, AFGBuildableWire* Wire)
		{
			OnAddWireConnectionEnter(Self, Wire);
		});
	...
	SUBSCRIBE_METHOD(
		UFGCircuitConnectionComponent::RemoveConnection,
		[](auto& /*Scope*/, UFGCircuitConnectionComponent* Self, AFGBuildableWire* Wire)
		{
			OnRemoveWireConnectionEnter(Self, Wire);
		});
	...
	SUBSCRIBE_METHOD_AFTER(
		...
		[](AFGBuildableCircuitSwitch* Switch, bool bOn)
		{
			OnSetSwitchOn(Switch, bOn);
		});
	...
	SUBSCRIBE_METHOD_VIRTUAL_AFTER(
		...
		{
			OnBridgeCircuitsRebuilt(Bridge);
		});
