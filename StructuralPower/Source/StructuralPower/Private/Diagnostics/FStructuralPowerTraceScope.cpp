	...
	, Detail(InDetail)
{
	...
	if (Detail.IsEmpty())
	{
		UE_LOG(LogStructuralPower, Log,
			TEXT("[HALSP] trace> %s.%s frame=%d depth=%d"),
			Domain,
			Op,
	...
	{
		UE_LOG(LogStructuralPower, Log,
			TEXT("[HALSP] trace> %s.%s frame=%d depth=%d %s"),
			Domain,
			Op,
			FStructuralPowerTraceUtil::GetFrameNumber(),
			Depth,
...
FStructuralPowerTraceScope::~FStructuralPowerTraceScope()
{
	...
	{
		UE_LOG(LogStructuralPower, Log,
			TEXT("[HALSP] trace< %s.%s ms=%.3f frame=%d depth=%d"),
			Domain,
			Op,
			ElapsedMs,
			FStructuralPowerTraceUtil::GetFrameNumber(),
	...
	{
		UE_LOG(LogStructuralPower, Log,
			TEXT("[HALSP] trace< %s.%s ms=%.3f frame=%d depth=%d %s"),
			Domain,
			Op,
			ElapsedMs,
			FStructuralPowerTraceUtil::GetFrameNumber(),
