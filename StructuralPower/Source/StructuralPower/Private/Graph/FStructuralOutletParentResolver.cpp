...
namespace {
static void ConsiderCandidate(const FBox& Bounds, TSubclassOf<AFGBuildable> BuildableClass,
                              const AFGBuildable* Outlet, FStructuralWallAnchor Candidate,
                              FStructuralWallAnchor& BestPreferred, float& BestPreferredScore,
                              FStructuralWallAnchor& BestAny, float& BestAnyScore) {
  ...
  const float Score = FStructuralOutletParentHeuristics::ScoreParentCandidate(
      AnchorLocation, Bounds, BuildableClass, Outlet);
  ...
  const bool bPreferredClass =
      CDO && (bPreferFoundation ? FStructuralOutletParentHeuristics::IsPreferredFoundationClass(CDO)
...
static FStructuralWallAnchor TryResolveAttachedLightweightParent(AFGBuildable* Outlet,
                                                                 UWorld* World) {
  ...
  for (const TPair<TSubclassOf<AFGBuildable>, TArray<FRuntimeBuildableInstanceData>>& ClassPair :
    ...
    for (int32 Index = 0; Index < ClassPair.Value.Num(); ++Index) {
      ...
      FBox Bounds =
          RuntimeData.BoundingBox.IsValid
      ...
      if (!Bounds.IsValid) {
        ...
        Bounds = FBox(RuntimeData.Transform.GetLocation() - HalfExtents,
                      RuntimeData.Transform.GetLocation() + HalfExtents);
      }
      ...
      const float Score = FStructuralOutletParentHeuristics::ScoreParentCandidate(
          AnchorLocation, Bounds, ClassPair.Key, Outlet);
      if (Score >= BestScore) {
        continue;
      }
  ...
  if (Best.IsValid()) {
    ...
    UE_LOG(LogStructuralPower, Verbose,
           ...
           *Outlet->GetName(), *Best.Lightweight.BuildableClass->GetName(), Best.Lightweight.Index,
           DistCm);
  }
...
FindParentFromLiveBusMembers(AFGBuildable* Outlet,
  ...
  if (AFGBuildableSubsystem* BuildableSubsystem = AFGBuildableSubsystem::Get(Outlet->GetWorld())) {
    const FVector AnchorLocation =
        FStructuralOutletParentHeuristics::GetOutletAnchorLocation(Outlet);
    ...
    for (AFGBuildable* Candidate : BuildableSubsystem->GetAllBuildablesRef()) {
      ...
      Anchor.WorldLocation =
          Bounds.IsValid ? Bounds.GetClosestPointTo(AnchorLocation) : Candidate->GetActorLocation();

      ConsiderCandidate(Bounds, Candidate->GetClass(), Outlet, Anchor, BestPreferred,
                        BestPreferredScore, BestAny, BestAnyScore);
    }
    ...
    if (Result.IsValid()) {
      UE_LOG(LogStructuralPower, Log,
             ...
             *Outlet->GetName(), IsValid(Result.Actor) ? *Result.Actor->GetName() : TEXT("null"),
             Result.Lightweight.IsValid() ? *Result.Lightweight.BuildableClass->GetName()
                                          : TEXT("null"),
...
TryResolveFromStructureGraph(AFGBuildable* Outlet, const FStructuralConnectivityGraph* Graph,
  ...
  if (BestNodeId.IsLightweight()) {
    TSubclassOf<AFGBuildable> BuildableClass =
        BestNodeId.BuildableClass.TryLoadClass<AFGBuildable>();
...
static void LogResolvedParent(AFGBuildable* Outlet,
                              const FStructuralOutletParentResolveResult& Result) {
  if (!Result.Anchor.IsValid()) {
    return;
  }

  UE_LOG(LogStructuralPower, Verbose, TEXT("[HALSP] outlet %s parent via %s actor=%s lw=%s[%d]"),
         *Outlet->GetName(), FStructuralOutletParentResolver::FormatParentMethod(Result.Method),
         IsValid(Result.Anchor.Actor) ? *Result.Anchor.Actor->GetName() : TEXT("null"),
         Result.Anchor.Lightweight.IsValid() ? *Result.Anchor.Lightweight.BuildableClass->GetName()
                                             : TEXT("null"),
...
FStructuralOutletParentResolveResult FStructuralOutletParentResolver::ResolveDetailed(
  ...
  if (Params.LightweightIndex) {
    if (FStructuralWallAnchor IndexAnchor =
            Params.LightweightIndex->FindParentWallForOutlet(Outlet);
        IndexAnchor.IsValid()) {
  ...
  if (FStructuralWallAnchor GraphAnchor =
          TryResolveFromStructureGraph(Outlet, Params.StructureGraph, Params);
      GraphAnchor.IsValid()) {
  ...
  if (Params.bAllowLiveScan) {
    if (FStructuralWallAnchor LiveAnchor =
            FindParentFromLiveBusMembers(Outlet, Params.BusMemberIndex);
        LiveAnchor.IsValid()) {
  ...
  const bool bIndexedSpatialExhausted =
      Params.LightweightIndex && Params.BusMemberIndex && Params.StructureGraph;
...
FBox FStructuralOutletParentResolver::BoundsForStructuralAnchor(const FStructuralWallAnchor& Anchor,
                                                                UWorld* World) {
...
bool FStructuralOutletParentResolver::IsStructurallyAnchored(
    const FStructuralOutletParentResolveResult& Result, AFGBuildable* Outlet) {
