...
bool FStructuralHostAttachAdapter::ConfirmSiteAttach(
    const FStructuralOutletParentResolveResult& ParentResolve, AFGBuildable* Endpoint) {
...
FBox FStructuralHostAttachAdapter::ExpandHostBounds(const FBox& Bounds,
  ...
  if (HostKind == EStructuralHostKind::Foundation ||
    ...
    return Bounds.ExpandBy(StructuralPowerConstants::StructuralConnectivityGapCm)
        .ExpandBy(FVector(0.0, 0.0, StructuralPowerConstants::GroundPoleFoundationVerticalReachCm));
  }
