...
bool FStructuralGraphCircuitEcho::ShouldSkipPanelCircuitEcho(AFGBuildableLightsControlPanel* Panel,
  ...
  if (!ChannelKey.Source.IsNone() && Root != INDEX_NONE &&
  ...
  if (bRoutingUnchanged) {
    ...
    if (FStructuralPanelPortResolver::Resolve(Panel, Ports)) {
      ...
      if (FStructuralCircuitPromotionUtil::ConnectorSuppliesPower(InputPower) ||
          FStructuralPanelAttach::SupplyAlreadyLinked(*Session, Panel, Ports, Root, ChannelKey)) {
...
bool FStructuralGraphCircuitEcho::ShouldSkipSwitchCircuitEcho(AFGBuildableCircuitSwitch* Switch,
                                                              const TCHAR** OutReason) {
  auto SetReason = [OutReason](const TCHAR* Reason) {
    if (OutReason) {
      *OutReason = Reason;
  ...
  if (Tracked && Tracked->MountParentId.IsValid()) {
    ...
    const FStructuralWallAnchor ParentAnchor =
        Session->BridgeRootIndex().ResolveOutletAnchor(Switch);
...
void FStructuralGraphCircuitEcho::MarkEchoDirtyForSwitchToggle(AFGBuildableCircuitSwitch* Switch,
  ...
  if (FStructuralPowerModConfig::IsGroupLightingEnabled() && IsValid(Switch) &&
...
void FStructuralGraphCircuitEcho::NotePanelCircuitEchoProcessed(
...
void FStructuralGraphCircuitEcho::NoteSwitchCircuitEchoProcessed(
