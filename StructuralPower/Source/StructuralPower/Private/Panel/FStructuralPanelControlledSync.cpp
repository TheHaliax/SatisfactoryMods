...
FArrayProperty* GetControlledBuildablesProperty() {
  return FindFProperty<FArrayProperty>(AFGBuildableControlPanelHost::StaticClass(),
                                       TEXT("mControlledBuildables"));
}

bool ControlledSetsMatch(const TArray<AFGBuildable*>& Desired,
...
void WriteControlledBuildables(AFGBuildableControlPanelHost* Host,
                               const TArray<AFGBuildable*>& Buildables) {
  FArrayProperty* Prop = GetControlledBuildablesProperty();
  FObjectProperty* Inner = Prop ? CastField<FObjectProperty>(Prop->Inner) : nullptr;
...
bool PanelHasStructuralHiddenPath(FStructuralGraphSession& Session,
                                  AFGBuildableLightsControlPanel* Panel, int32 Root,
  ...
  UFGPowerConnectionComponent* Feed =
      Session.Circuit().ResolveSubnetFeedConnector(Root, ChannelKey);
...
bool ShouldOverrideControlledBuildables(FStructuralGraphSession& Session,
                                        AFGBuildableLightsControlPanel* Panel, int32 Root,
                                        const FStructuralChannelKey& ChannelKey,
                                        const FStructuralPanelPorts& Ports) {
...
void FStructuralPanelControlledSync::ApplyKeyedSubnet(FStructuralGraphSession& Session,
  ...
  UE_LOG(LogStructuralPower, Verbose, TEXT("[HALSP] panel %s keyed controlled=%d control=%s"),
         *Panel->GetName(), KeyedLights.Num(), *EffectiveControl.ToString());
}
...
void FStructuralPanelControlledSync::MirrorSharedControlState(
  ...
  for (const FStructuralNodeId& PeerId : *PanelIds) {
    ...
    const bool bSameBank =
        Control.IsNone() || Control == StructuralPowerConstants::ControlUnconfigured
