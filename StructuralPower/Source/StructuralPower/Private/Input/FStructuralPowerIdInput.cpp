...
void FStructuralPowerIdInput::OnPlayerInputInitialized(AFGCharacterPlayer* Player,
...
void FStructuralPowerIdInput::OpenIdPanelForController(AFGPlayerController* PlayerController) {
  ...
  if (FStructuralPowerIdPresenterFactory::IsAnyPanelVisible()) {
    ...
    if (IsValid(Target) && CurrentTarget != Target) {
      ...
      UE_LOG(LogStructuralPower, Log, TEXT("[HALSP] Id panel retargeted via I key target=%s"),
             *Target->GetName());
    } else {
      Presenter.Close();
