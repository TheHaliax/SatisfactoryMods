...
struct FStructuralHoverpackPropertyCache {
  ...
  void Ensure(const UClass* Class) {
    ...
    DisconnectionTime =
        FindFProperty<FFloatProperty>(Class, TEXT("mPowerConnectionDisconnectionTime"));
...
FCylindricalTetherMetrics ComputeCylindricalTetherMetrics(const FVector& OwnerLoc,
                                                          const FVector& AnchorLoc,
                                                          float MaxDistance) {
  FCylindricalTetherMetrics Metrics;
  Metrics.Horizontal =
      FVector2D::Distance(FVector2D(OwnerLoc.X, OwnerLoc.Y), FVector2D(AnchorLoc.X, AnchorLoc.Y));
...
bool IsWithinCylindricalTetherRange(const FCylindricalTetherMetrics& Metrics, float MaxHorizontal,
                                    float MaxVertical) {
  return MaxHorizontal > 0.0f && MaxVertical > 0.0f && Metrics.Horizontal <= MaxHorizontal &&
         Metrics.Vertical <= MaxVertical;
}

float NormalizedCylindricalTetherRange(const FCylindricalTetherMetrics& Metrics,
  ...
  const float Normalized =
      FMath::Max(Metrics.Horizontal / MaxHorizontal, Metrics.Vertical / MaxVertical);
...
bool ReadConnectionLocation(const AFGHoverPack* Pack, FVector& OutLocation) {
  ...
  if (const FVector* Ptr =
          GHoverpackProps.ConnectionLocation->ContainerPtrToValuePtr<FVector>(Pack)) {
    OutLocation = *Ptr;
    return true;
  }
...
FCylindricalTetherMetrics SessionTetherMetrics(const AFGHoverPack* Pack,
...
void FStructuralHoverpackBridge::RegisterHooks() {
  ...
  SUBSCRIBE_METHOD_VIRTUAL(AFGHoverPack::ConnectToNearestPowerConnection, Sample,
  ...
  SUBSCRIBE_METHOD_VIRTUAL(
      AFGHoverPack::Tick, Sample,
      [](auto& /*Scope*/, AFGHoverPack* Pack, float DeltaTime) { BeforeTick(Pack, DeltaTime); });

  SUBSCRIBE_METHOD_VIRTUAL_AFTER(
      AFGHoverPack::Tick, Sample,
      [](AFGHoverPack* Pack, float DeltaTime) { AfterTick(Pack, DeltaTime); });

  SUBSCRIBE_METHOD_VIRTUAL_AFTER(AFGHoverPack::UnEquip, Sample,
                                 [](AFGHoverPack* Pack) { OnUnEquip(Pack); });

  SUBSCRIBE_METHOD_VIRTUAL(
      AFGHoverPack::GetMaxDistanceFromCurrentConnection, Sample,
  ...
  SUBSCRIBE_METHOD_VIRTUAL(
      AFGHoverPack::GetDistanceFromCurrentConnection, Sample,
      [](auto& Scope, const AFGHoverPack* Pack) {
        FVector Anchor = FVector::ZeroVector;
        float MaxHorizontal = 0.0f;
        float MaxVertical = 0.0f;
  ...
  SUBSCRIBE_METHOD_VIRTUAL(
      AFGHoverPack::GetCurrentConnectionLocation, Sample,
      [](auto& Scope, const AFGHoverPack* Pack) {
        FVector Anchor = FVector::ZeroVector;
        float MaxHorizontal = 0.0f;
        float MaxVertical = 0.0f;
  ...
  SUBSCRIBE_METHOD_VIRTUAL(
      AFGHoverPack::GetNormalizedDistanceFromConnection, Sample,
      [](auto& Scope, const AFGHoverPack* Pack) {
        FVector Anchor = FVector::ZeroVector;
        float MaxHorizontal = 0.0f;
        float MaxVertical = 0.0f;
        ...
        if (ResolvePublishedTether(Pack, Anchor, MaxHorizontal, MaxVertical, Metrics)) {
          ...
          Scope.Override(NormalizedCylindricalTetherRange(Cylindrical, MaxHorizontal, MaxVertical));
        }
      });

  SUBSCRIBE_METHOD_VIRTUAL(
      AFGHoverPack::GetHeightAboveCurrentConnection, Sample,
      [](auto& Scope, const AFGHoverPack* Pack) {
        FVector Anchor = FVector::ZeroVector;
        float MaxHorizontal = 0.0f;
        float MaxVertical = 0.0f;
        FStructuralHoverpackTetherMetrics Metrics;
        if (ResolvePublishedTether(Pack, Anchor, MaxHorizontal, MaxVertical, Metrics)) {
          Scope.Override(FMath::Clamp(Metrics.Vertical / MaxVertical, 0.0f, 1.0f));
        }
      });
}
...
float FStructuralHoverpackBridge::GetVanillaFieldRadius(
    const FStructuralHoverpackSession& Session) {
...
void FStructuralHoverpackBridge::LogTransition(const AFGHoverPack* Pack, const TCHAR* Event,
                                               const FStructuralHoverpackSession& Session,
                                               float Distance) {
  ...
  UE_LOG(LogStructuralPower, Log,
         TEXT("[HALSP] hoverpack %s %s src=%d root=%d h=%.0f v=%.0f lim=%.0f maxH=%.0f maxV=%.0f "
              "feed=%s circuit=%d hasPower=%d"),
         *Pack->GetName(), Event, static_cast<int32>(Session.Source), Session.StructuralRoot,
         Metrics.Horizontal, Metrics.Vertical, Metrics.Limiting, Session.MaxHorizontal,
         Session.MaxVertical, Feed ? *Feed->GetName() : TEXT("-"),
         Feed ? Feed->GetCircuitID() : INDEX_NONE, Feed && Feed->HasPower() ? 1 : 0);
}
...
bool FStructuralHoverpackBridge::SyncPowerFeed(AFGHoverPack* Pack,
  ...
  if (Pack->GetCurrentPowerConnection() == Feed && Pack->HasConnection() &&
      PowerInfo->IsConnected() && PowerInfo->GetTargetConsumption() == Consumption) {
    return true;
  }
...
void FStructuralHoverpackBridge::SyncStructuralVirtualTether(AFGHoverPack* Pack,
...
void FStructuralHoverpackBridge::PublishStructuralTetherToOwningClient(
    AFGHoverPack* Pack, FStructuralHoverpackSession& Session, bool bClear) {
  ...
  const bool bUnchanged =
      Session.bClientTetherPublished &&
...
void FStructuralHoverpackBridge::ApplyClientTetherMirror(const FVector& Anchor, float MaxHorizontal,
                                                         float MaxVertical) {
...
bool FStructuralHoverpackBridge::TryGetClientTetherMirror(FVector& OutAnchor,
...
void FStructuralHoverpackBridge::SyncReplicationFields(AFGHoverPack* Pack,
                                                       FStructuralHoverpackSession& Session) {
...
void FStructuralHoverpackBridge::ClearSession(AFGHoverPack* Pack, bool bDisconnectPower,
                                              const TCHAR* Event) {
  if (!IsValid(Pack)) {
    return;
  }
...
bool FStructuralHoverpackBridge::FindNearestVisibleConnector(
    AFGHoverPack* Pack, float MaxDistance, UFGPowerConnectionComponent* PreferredFeed,
    UFGPowerConnectionComponent*& OutFeed, FVector& OutAnchor, float& OutDistance) {
  OutFeed = nullptr;
  ...
  if (IsValid(PreferredFeed) && PreferredFeed != OwnConn && PreferredFeed->GetWorld() == World &&
    ...
    const FCylindricalTetherMetrics PreferredMetrics =
        ComputeCylindricalTetherMetrics(OwnerLoc, PreferredAnchor, MaxDistance);
  ...
  for (TObjectIterator<UFGPowerConnectionComponent> It; It; ++It) {
    ...
    const FCylindricalTetherMetrics Metrics =
        ComputeCylindricalTetherMetrics(OwnerLoc, Anchor, MaxDistance);
    if (!IsWithinCylindricalTetherRange(Metrics, MaxDistance, MaxDistance)) {
      continue;
    }

    if (!Best || Metrics.Limiting + HoverpackFeedStickinessDistance < BestLimiting) {
...
bool FStructuralHoverpackBridge::FindStructuralTether(AFGHoverPack* Pack, float MaxHorizontal,
                                                      float MaxVertical,
                                                      UFGPowerConnectionComponent*& OutFeed,
                                                      FVector& OutAnchor, int32& OutRoot,
                                                      float& OutDistance) {
  OutFeed = nullptr;
  OutAnchor = FVector::ZeroVector;
  OutRoot = INDEX_NONE;
  ...
  AStructuralPowerGraphSubsystem* Graph =
      IsValid(World) ? AStructuralPowerGraphSubsystem::Find(World) : nullptr;
  if (!Graph) {
    return false;
  }
  ...
  const FCylindricalTetherMetrics Metrics =
      ComputeCylindricalTetherMetrics(OwnerLoc, OutAnchor, 0.0f);
...
bool FStructuralHoverpackBridge::ApplySession(AFGHoverPack* Pack,
                                              FStructuralHoverpackSession& Session,
                                              EStructuralHoverpackTetherSource Source,
                                              UFGPowerConnectionComponent* Feed,
                                              const FVector& Anchor, int32 StructuralRoot,
                                              float Distance, bool bLogTransition) {
  if (!IsValid(Feed) || !Feed->HasPower()) {
    return false;
  }

  const bool bSourceChanged = Session.Source != Source || Session.Feed.Get() != Feed ||
  ...
  const FStructuralHoverpackAxisLimits Limits =
      GetSessionAxisLimits(Source, Session.BaseSearchRadius);
...
bool FStructuralHoverpackBridge::ResolveTether(AFGHoverPack* Pack) {
  ...
  const float BaseRadius =
      Session.BaseSearchRadius > 0.0f ? Session.BaseSearchRadius : GetBaseSearchRadius(Pack);
  const FStructuralHoverpackAxisLimits VisibleLimits =
      GetSessionAxisLimits(EStructuralHoverpackTetherSource::VisibleConnector, BaseRadius);
  const FStructuralHoverpackAxisLimits StructuralLimits =
      GetSessionAxisLimits(EStructuralHoverpackTetherSource::StructuralGeometry, BaseRadius);
  ...
  const bool bHasVisible = FindNearestVisibleConnector(Pack, VisibleMax, PreferredFeed, VisibleFeed,
                                                       VisibleAnchor, VisibleDistance);
  ...
  const bool bHasStructural =
      FindStructuralTether(Pack, StructuralLimits.Horizontal, StructuralLimits.Vertical,
                           StructuralFeed, StructuralAnchor, StructuralRoot, StructuralDistance);
  ...
  if (Session.Source != EStructuralHoverpackTetherSource::None) {
    ...
    if (ValidateAndRefreshSession(Pack, Session)) {
      ...
      if (ShouldPublishSessionToVanilla(Pack, Session) &&
          CurrentMetrics.Limiting <= Challenger.Distance + HoverpackFeedStickinessDistance) {
        SyncReplicationFields(Pack, Session);
        return true;
      }
    }
  ...
  return ApplySession(Pack, Session, Challenger.Source, Challenger.Feed, Challenger.Anchor,
                      Challenger.StructuralRoot, Challenger.Distance,
                      /*bLogTransition=*/true);
}

bool FStructuralHoverpackBridge::ValidateAndRefreshSession(AFGHoverPack* Pack,
                                                           FStructuralHoverpackSession& Session) {
  if (Session.Source == EStructuralHoverpackTetherSource::None) {
    return false;
  }
  ...
  if (Session.Source == EStructuralHoverpackTetherSource::StructuralGeometry) {
    ...
    if (FindStructuralTether(Pack, Session.MaxHorizontal, Session.MaxVertical, RefreshedFeed,
                             RefreshedAnchor, RefreshedRoot, RefreshedDistance) &&
        RefreshedFeed == Feed) {
...
void FStructuralHoverpackBridge::MaintainSession(AFGHoverPack* Pack, float DeltaTime) {
  ...
  const bool bShouldSearch =
      Session.SearchTimer >= TickRate || Session.Source == EStructuralHoverpackTetherSource::None;
...
bool FStructuralHoverpackBridge::ShouldPublishSessionToVanilla(
    const AFGHoverPack* Pack, const FStructuralHoverpackSession& Session) {
  if (Session.Source == EStructuralHoverpackTetherSource::None || Session.MaxHorizontal <= 0.0f ||
...
bool FStructuralHoverpackBridge::ResolvePublishedTether(
    const AFGHoverPack* Pack, FVector& OutAnchor, float& OutMaxHorizontal, float& OutMaxVertical,
  ...
  if (const UFGPowerConnectionComponent* Feed = Pack->GetCurrentPowerConnection()) {
    ...
    if (!bHasClientMirror) {
      ...
      const FStructuralHoverpackAxisLimits Limits = GetSessionAxisLimits(
          EStructuralHoverpackTetherSource::StructuralGeometry, GetBaseSearchRadius(Pack));
  ...
  const FCylindricalTetherMetrics Metrics =
      ComputeCylindricalTetherMetrics(GetOwnerLocation(Pack), OutAnchor, 0.0f);
  OutMetrics.Horizontal = Metrics.Horizontal;
  OutMetrics.Vertical = Metrics.Vertical;
  OutMetrics.Limiting = Metrics.Limiting;
  OutMetrics.StraightLine = Metrics.StraightLine;
