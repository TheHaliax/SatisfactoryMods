...
void UStructuralPowerRCO::GetLifetimeReplicatedProps(
...
void UStructuralPowerRCO::Server_SetEndpointIds_Implementation(
    AFGBuildable* Buildable, FName Source, FName Control, bool bClearSource, bool bClearControl,
    const bool bGlobalControl, const bool bTouchGlobalControl) {
  ...
  Graph->SetEndpointIds(Buildable, Source, Control, bClearSource, bClearControl, bGlobalControl,
                        bTouchGlobalControl);

  UE_LOG(LogStructuralPower, Log,
         TEXT("[HALSP] RCO SetEndpointIds %s src=%s ctl=%s clearSrc=%d clearCtl=%d"
              " global=%d touchGlobal=%d"),
         *Buildable->GetName(), bClearSource ? TEXT("(clear)") : *Source.ToString(),
         bClearControl ? TEXT("(clear)") : *Control.ToString(), bClearSource ? 1 : 0,
         bClearControl ? 1 : 0, bGlobalControl ? 1 : 0, bTouchGlobalControl ? 1 : 0);
}

void UStructuralPowerRCO::Server_RequestComponentIdList_Implementation(
    AFGBuildable* ContextBuildable) {
...
void UStructuralPowerRCO::Client_ReceiveComponentIdList_Implementation(
  ...
  UE_LOG(LogStructuralPower, Log,
         TEXT("[HALSP] RCO ComponentIdList target=%s sources=%d controls=%d switchCtl=%d "
              "lightGrp=%d resolvedSrc=%s resolvedCtl=%s"),
         List.DefaultSourceId.IsNone() ? TEXT("?") : *List.DefaultSourceId.ToString(),
         List.NamedSourceIds.Num(), List.NamedControlIds.Num(), List.NamedSwitchControlIds.Num(),
...
void UStructuralPowerRCO::Client_SyncHoverpackTether_Implementation(const FVector& Anchor,
                                                                    float MaxHorizontal,
                                                                    float MaxVertical) {
