...
void LogHook(AFGBuildable* Buildable, const TCHAR* Hook, const TCHAR* Action,
             const TCHAR* Detail = nullptr);
void LogPlacementSkip(AFGBuildable* Buildable, const TCHAR* Reason,
                      ELogVerbosity::Type Verbosity = ELogVerbosity::Verbose);
void LogLinkOp(const TCHAR* Op, UFGCircuitConnectionComponent* A, UFGCircuitConnectionComponent* B,
               bool bSuccess, const TCHAR* Path,
               ELogVerbosity::Type Verbosity = ELogVerbosity::Log);

void LogConnector(const TCHAR* Role, AFGBuildable* Owner, UFGCircuitConnectionComponent* Connector);

void LogLightConsumer(AFGBuildableLightSource* Light, int32 Root, bool bParentValid,
                      const FStructuralChannelKey& Key, UFGPowerConnectionComponent* Plug,
                      const TCHAR* Path, int32 PanelSupplyReady = -1,
                      int32 PanelDownstreamFed = -1);

void LogPanelConsumer(AFGBuildableLightsControlPanel* Panel, int32 Root,
                      const FStructuralChannelKey& Key, const FStructuralPanelPorts& Ports,
                      bool bSupplyReady, int32 ControlledCount,
