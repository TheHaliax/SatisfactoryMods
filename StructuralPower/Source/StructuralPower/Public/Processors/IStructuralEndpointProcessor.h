...
class STRUCTURALPOWER_API IStructuralEndpointProcessor {
 public:
  ...
  virtual void ProcessPlacement(FStructuralPowerContext& Ctx, AFGBuildable* Host,
                                bool bLocalPromoteOnly = false) = 0;

  void Process(FStructuralPowerContext& Ctx, AFGBuildable* Host, bool bLocalPromoteOnly = false) {
