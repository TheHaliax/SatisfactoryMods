...
struct STRUCTURALPOWER_API FStructuralNodeId
{
	...
	bool IsValid() const
	{
		return !BuildableClass.IsNull()
			&& (!ActorName.IsNone() || LightweightIndex != INDEX_NONE);
	}

	bool IsLightweight() const { return LightweightIndex != INDEX_NONE && ActorName.IsNone(); }
	...
	{
		return HashCombine(
			HashCombine(GetTypeHash(Id.BuildableClass), GetTypeHash(Id.ActorName)),
