...
class STRUCTURALPOWER_API FStructuralSiteMembership {
 public:
  static bool ResolveSiteContext(FStructuralGraphSession& Session, AFGBuildable* Endpoint,
                                 FStructuralSiteContext& OutSite,
                                 bool bUsePoleRootResolver = false);

  static void RegisterOnBulkLoad(FStructuralGraphSession& Session, AFGBuildable* Host,
                                 EStructuralEndpointKind Kind, FTrackedEndpoint& Tracked,
  ...
  static int32 SiteRootFromMount(FStructuralGraphSession& Session,
