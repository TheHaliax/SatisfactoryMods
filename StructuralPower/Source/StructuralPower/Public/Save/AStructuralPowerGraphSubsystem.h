...
class STRUCTURALPOWER_API AStructuralPowerGraphSubsystem : public AInfo, public IFGSaveInterface {
 ...
 public:
  ...
  bool ShouldSkipPanelCircuitEcho(AFGBuildableLightsControlPanel* Panel,
                                  const TCHAR** OutReason = nullptr);
  bool ShouldSkipSwitchCircuitEcho(AFGBuildableCircuitSwitch* Switch,
                                   const TCHAR** OutReason = nullptr);
  ...
  TArray<FStructuralNodeId> GetControlIdGangMembers(const FStructuralComponentKey& ComponentKey,
                                                    FName ControlId) const;

  int32 GetStructureNodeCount() const {
    return StructureGraph.GetNodeCount();
  }
  int32 GetTrackedPoleCount() const {
    return TrackedEndpoints.Num();
  }
  int32 GetTrackedLightweightCount() const {
    return LightweightIndex.GetTrackedCount();
  }
  int32 GetPendingJobCount() const {
    return PlacementQueue.GetPendingCount();
  }
  bool IsBuildablePlacementPending(AFGBuildable* Buildable) const;
  void GetGraphStats(int32& OutComponents, int32& OutLargest) {
    StructureGraph.GetComponentStats(OutComponents, OutLargest);
  }
  ...
  bool ShouldDeferSwitchCircuitRefresh() const {
    return CircuitPromotionDepth > 0;
  }

  bool IsBulkLoadDrainActive() const {
    return bBulkLoadDrainActive;
  }
  bool IsPostLoadRebuilt() const {
    return bPostLoadRebuilt;
  }
  bool HasPendingBulkRemesh() const {
    return BulkDrainOps.HasPendingRemesh();
  }
  bool IsPendingPostLoadLightReconcile() const {
    return bPendingPostLoadLightReconcile;
  }
  bool IsPendingPostLoadMachineReconcile() const {
    return bPendingPostLoadMachineReconcile;
  }
  bool IsPendingFinalLightingReconcile() const {
    return bPendingFinalLightingReconcile;
  }
  bool IsPendingStructureSplitReconcile() const {
    return bPendingStructureSplitReconcile;
  }
 ...
 private:
  ...
  bool bPendingPostLoadMachineReconcile = false;
  bool bPendingFinalLightingReconcile = false;
  bool bPendingStructureSplitReconcile = false;
  TSet<int32> PendingStructureSplitAffectedRoots;
  /** Structure UF deletes batched until split reconcile (mass dismantle). */
