...
void FFluidAppearanceObserver::RegisterHooks()
{
	...
	SUBSCRIBE_METHOD_VIRTUAL_AFTER(
		AFGBuildablePipeline::OnFluidDescriptorSet,
		GetMutableDefault<AFGBuildablePipeline>(),
		[](AFGBuildablePipeline* Pipe)
		{
			EnqueueBuildable(Pipe);
		});

	SUBSCRIBE_METHOD_VIRTUAL_AFTER(
		AFGBuildablePipelinePump::OnFluidDescriptorSet,
		GetMutableDefault<AFGBuildablePipelinePump>(),
		[](AFGBuildablePipelinePump* Pump)
		{
			EnqueueBuildable(Pump);
		});
