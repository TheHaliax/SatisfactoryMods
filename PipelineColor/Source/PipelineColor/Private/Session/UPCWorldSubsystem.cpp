...
void UPCWorldSubsystem::BindSwatchStore(UWorld* World)
{
	...
	if (Store && BoundStore.Get() != Store)
	{
		...
		BoundStore = Store;
		StoreChangedHandle = Store->OnEntryChanged().AddUObject(
			this, &UPCWorldSubsystem::OnSwatchEntryChanged);
	}
...
void UPCWorldSubsystem::ScanWorld()
{
	...
	UE_LOG(LogPipelineColor, Log, TEXT("%s WorldReady scan enqueued %d"),
		PIPELINECOLOR_LOG_PREFIX, Found);
}
