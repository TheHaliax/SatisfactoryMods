...
void UPCFinish_MetallicColor::EnsureIconLoaded() {
  ...
  const FSoftClassPath ChromePath(
      TEXT("/Game/FactoryGame/Buildable/-Shared/Customization/PaintFinishes/Metals/"
           "PaintFinishDesc_Chrome.PaintFinishDesc_Chrome_C"));
  UClass* ChromeClass = ChromePath.TryLoadClass<UFGFactoryCustomizationDescriptor_PaintFinish>();
  if (!ChromeClass) {
    UE_LOG(LogPipelineColor, Warning, TEXT("%s EnsureIconLoaded: Chrome finish missing"),
  ...
  if (!Chrome) {
    UE_LOG(LogPipelineColor, Warning, TEXT("%s EnsureIconLoaded: Chrome CDO missing"),
           PIPELINECOLOR_LOG_PREFIX);
    return;
  }
