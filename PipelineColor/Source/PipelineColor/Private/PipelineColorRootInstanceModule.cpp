...
TSubclassOf<UFGCustomizerCategory>
	UPipelineColorRootInstanceModule::CachedCategory;
TSubclassOf<UFGCustomizerSubCategory>
	UPipelineColorRootInstanceModule::CachedSubCategory;
...
void UPipelineColorRootInstanceModule::CollectPcRecipes(
...
UPipelineColorRootInstanceModule::GetOrCreatePipelineColorCategory()
{
	...
	if (!Generated)
	{
		UE_LOG(LogPipelineColor, Error,
			TEXT("%s ClassGenerator failed for PipelineColor category"),
	...
	UE_LOG(LogPipelineColor, Log, TEXT("%s ClassGen top category %s"),
		PIPELINECOLOR_LOG_PREFIX, *GetNameSafe(Generated));
	return CachedCategory;
}
...
UPipelineColorRootInstanceModule::GetOrCreatePipelineColorSubCategory()
{
	...
	UClass* Generated = FClassGenerator::GenerateSimpleClass(
		TEXT("/PipelineColor"),
		TEXT("PC_SubCat_Default"),
		UPCSwatchSubCategory::StaticClass());
	if (!Generated)
	{
		UE_LOG(LogPipelineColor, Error,
			TEXT("%s ClassGenerator failed for PipelineColor subcategory"),
			PIPELINECOLOR_LOG_PREFIX);
	...
	UE_LOG(LogPipelineColor, Log, TEXT("%s ClassGen subcategory %s"),
		PIPELINECOLOR_LOG_PREFIX, *GetNameSafe(Generated));
	return CachedSubCategory;
}
	...
	TSubclassOf<UFGFactoryCustomizationDescriptor_Swatch> Swatch)
{
	...
	if (!Collection->mCustomizations.Contains(Swatch))
	{
		...
		UE_LOG(LogPipelineColor, Log, TEXT("%s customizer +%s"),
			PIPELINECOLOR_LOG_PREFIX, *GetNameSafe(Swatch));
	}
}
	...
	TSubclassOf<UFGFactoryCustomizationDescriptor_Swatch> Swatch)
{
	...
	{
		const FSoftClassPath Slot0Path(TEXT(
...
void UPipelineColorRootInstanceModule::UnlockPcSwatchesViaUnlockSubsystem(UWorld* World)
{
	...
	if (!UnlockSys)
	{
		UE_LOG(LogPipelineColor, Warning, TEXT("%s UnlockSubsystem missing"),
			PIPELINECOLOR_LOG_PREFIX);
		return;
	}
	...
	const FSoftObjectPath BuildGunPath(TEXT(
	...
	const FSoftClassPath CustomizerUnlockPath(TEXT(
		"/Game/FactoryGame/Unlocks/BP_UnlockCustomizer.BP_UnlockCustomizer_C"));
	if (UClass* CustomizerUnlockClass =
		CustomizerUnlockPath.TryLoadClass<UFGUnlockCustomizer>())
	{
		UFGUnlockCustomizer* CustomizerUnlock = NewObject<UFGUnlockCustomizer>(
			GetTransientPackage(), CustomizerUnlockClass);
		CustomizerUnlock->Apply(UnlockSys);
		UE_LOG(LogPipelineColor, Log, TEXT("%s BP_UnlockCustomizer applied"),
			PIPELINECOLOR_LOG_PREFIX);
	}
	else
	{
	...
	const FSoftClassPath RecipeUnlockPath(TEXT(
		"/Game/FactoryGame/Unlocks/BP_UnlockRecipe.BP_UnlockRecipe_C"));
	UClass* RecipeUnlockClass = RecipeUnlockPath.TryLoadClass<UFGUnlockRecipe>();
	if (!RecipeUnlockClass)
	{
		UE_LOG(LogPipelineColor, Error, TEXT("%s BP_UnlockRecipe missing"),
			PIPELINECOLOR_LOG_PREFIX);
		return;
	}
...
void UPipelineColorRootInstanceModule::DispatchLifecycleEvent(ELifecyclePhase Phase)
{
...
#if !WITH_EDITOR
		UE_LOG(LogPipelineColor, Display,
			TEXT("PipelineColor v1.0.0 — fluid-driven pipe swatches"));
#endif
	}
...
#if WITH_EDITOR
	UE_LOG(LogPipelineColor, Log, TEXT("%s skip runtime hooks in editor"),
		PIPELINECOLOR_LOG_PREFIX);
#else
	...
	SUBSCRIBE_METHOD_VIRTUAL_AFTER(
		AFGBuildablePipeline::BeginPlay,
