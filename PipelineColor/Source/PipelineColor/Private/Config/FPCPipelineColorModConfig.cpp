...
void FPCPipelineColorModConfig::ApplyDefaultFlags(bool bGasMetallic, bool bLiquidMetallic) {
  ...
  const bool bChanged = CVarDefaultGasMetallic.GetValueOnGameThread() != GasWant ||
...
bool FPCPipelineColorModConfig::TryApplySetCommand(const TArray<FString>& Args, UWorld* World) {
  ...
  if (IsValid(World) && World->GetNetMode() == NM_Client) {
    UE_LOG(LogPipelineColor, Warning, TEXT("%s PipelineColor.Set ignored on client"),
           PIPELINECOLOR_LOG_PREFIX);
    return false;
  }
  ...
  if (Key.Equals(TEXT("DefaultGasMetallic"), ESearchCase::IgnoreCase)) {
    ...
    UE_LOG(LogPipelineColor, Warning, TEXT("%s unknown config key: %s"), PIPELINECOLOR_LOG_PREFIX,
           *Key);
    return false;
  }
