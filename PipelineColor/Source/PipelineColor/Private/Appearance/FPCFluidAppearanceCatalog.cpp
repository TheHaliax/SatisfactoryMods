...
FPCFluidAppearanceCatalog::LoadFinish(const TCHAR* SoftPath, const TCHAR* Label) {
  ...
  if (!Loaded) {
    UE_LOG(LogPipelineColor, Warning, TEXT("%s catalog: failed load %s"), PIPELINECOLOR_LOG_PREFIX,
           Label);
  }
  return Loaded;
}
...
void FPCFluidAppearanceCatalog::BuildEntries() {
  FinishDefault = LoadFinish(TEXT("/Game/FactoryGame/Buildable/-Shared/Customization/PaintFinishes/"
                                  "PaintFinishDesc_Default.PaintFinishDesc_Default_C"),
                             TEXT("PaintFinishDesc_Default"));
  FinishMatte = LoadFinish(TEXT("/Game/FactoryGame/Buildable/-Shared/Customization/PaintFinishes/"
  ...
  UE_LOG(LogPipelineColor, Log, TEXT("%s catalog ready (%d fluids)"), PIPELINECOLOR_LOG_PREFIX,
         ByDescriptor.Num());
}
